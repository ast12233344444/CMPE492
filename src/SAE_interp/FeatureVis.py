...
def steer_feature_with_vae(
    ...
    for _ in pbar:
        ...
        if target_token_idx is not None:
            ...
            feature_act = current_encoded[:, :, feature_idx].mean(dim=1)

        # In your steer_feature_with_vae optimization loop:
        loss_maximize = -feature_act.mean()

        # Pull the latent back toward the center of the Gaussian (0)
        l2_penalty = torch.norm(latent_param, p=2)

        # Add it to the total loss
        total_loss = loss_maximize + (1 * l2_penalty)
        total_loss.backward()
        ...
        pbar.set_postfix({
            ...
            "l2 penalty": f"{l2_penalty.item():.4f}",
        })
